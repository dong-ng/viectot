...
// Xác định người dùng
$token = $_COOKIE['token'] ?? ($_SESSION['login'] ?? '');
if (!$token) {
    die("⚠️ Không tìm thấy token người dùng.");
}

$user = $db->get_row("SELECT * FROM users WHERE token = '" . addslashes($token) . "'");
if (!$user) {
    die("⚠️ Không xác thực được người dùng.");
}
$user_id = (int)$user['id'];

// Tìm admin
$admin = $db->get_row("SELECT * FROM users WHERE is_admin = 1 LIMIT 1");
if (!$admin) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fetch_messages'])) {
    ...
    $html = ob_get_clean();
    echo json_encode(['success' => true, 'html' => $html, 'user_name' => htmlspecialchars($admin['name'])]);
    exit();
}
